export default function SellerSearchProfile() {
  return {
    template: require('./search-profile.html'),
    restrict: 'E',
    // scope: {
...
SellerSearchProfileCtrl.$inject = ['$scope', 'FIRSTNEAR_CONSTANTS', 'localStorageService', '$state', '$stateParams', '$rootScope', 'BaseUrlService','$window','$location','CustomerDashboardService'];
  
function SellerSearchProfileCtrl($scope, FIRSTNEAR_CONSTANTS, localStorageService, $state, $stateParams, $rootScope, BaseUrlService,$window,$location, CustomerDashboardService) {
   var vm = this;
   vm.searchData = {
    service_kind : '1',
    service_location : '',
  ...
  $scope.$on('$viewContentLoaded', function() {
    CustomerDashboardService.getHeaderProfileData().then( function(res){       
